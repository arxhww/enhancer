...
class RegistryAction(Action):
    ...
    def rollback(self, snapshot: ActionSnapshot) -> None:
        ...
        if meta["value_existed"]:
            ...
            if not meta["subkey_existed"]:
                if self._is_subkey_empty(meta["path"]):
    ...
    def from_snapshot(cls, snapshot: ActionSnapshot) -> 'RegistryAction':
        if snapshot.action_type != "registry":
            raise ValueError(f"Expected registry snapshot, got {snapshot.action_type}")
        ...
        definition = {
            ...
            "value_type": meta.get("old_type", "DWORD"),
            "force_create": True
        }
        
        instance = cls(definition)
        instance._snapshot_meta = meta
